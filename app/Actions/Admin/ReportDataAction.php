...
final class ReportDataAction
{
    ...
    public function normalize(Collection $rows, string $report): Collection
    {
        ...
        return $rows->map(function ($row) use ($report, &$previousCapital): array {
            ...
            return match ($report) {
                'participants' => ['name' => trim($row->first_name . ' ' . $row->last_name) ?: $row->username, 'username' => $row->username, 'capital' => $this->moneyDecimal($row->capital_share), 'ratio' => $this->ratioPercent($row->capital_share, $row->snapshot_total), 'status' => $row->status, 'created_at' => optional($row->created_at)->format('Y-m-d')],
