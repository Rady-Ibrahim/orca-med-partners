...
final class Phase10SettlementSafetyTest extends TestCase
{
    ...
    public function test_payment_source_is_explicit_and_adjustment_preserves_paid_history(): void
    {
        ...
        $adjustment->update(['amount' => '20.00']);
        $this->assertDatabaseHas('settlement_adjustments', ['id' => $adjustment->id, 'amount' => '20.00']);
    }

    private function createAnnualData(Admin $admin, Participant $participant, int $year): void
    {
        $snapshot = CapitalSnapshot::query()->create(['snapshot_date' => $year.'-01-01', 'year' => $year, 'month' => 1, 'total_capital' => '1000.00', 'status' => 'final']);
        ...
        $rule = DistributionRule::query()->create(['effective_from' => $year.'-01-01', 'management_fee_rate' => '0.2500', 'depreciation_fund_rate' => '0.0500', 'growth_fund_rate' => '0.0250', 'incentive_fund_rate' => '0.0250', 'distributed_share_rate' => '0.6500', 'status' => 'active']);
        $profit = MonthlyProfit::query()->create(['capital_snapshot_id' => $snapshot->id, 'distribution_rule_id' => $rule->id, 'year' => $year, 'month' => 1, 'version' => 1, 'status' => 'approved', 'gross_profit' => '100.00', 'management_amount' => '25.00', 'depreciation_amount' => '5.00', 'growth_amount' => '2.50', 'incentive_amount' => '2.50', 'distributed_amount' => '65.00', 'rounding_delta_adjustment' => '0.00']);
        ParticipantProfitAllocation::query()->create(['monthly_profit_id' => $profit->id, 'participant_id' => $participant->id, 'amount' => '65.00', 'share_ratio' => '1.0000', 'status' => 'approved']);
    }
}
